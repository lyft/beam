  ...
  class FlinkRunnerTest(portable_runner_test.PortableRunnerTest):
    ...
    def test_metrics(self):
      ...
      with Pipeline(self.get_runner(), options) as p:
        # pylint: disable=expression-not-assigned
        (p
         | "source" >> FlinkStreamingImpulseSource()
         .set_interval_ms(0).set_message_count(110)
         | "mapper" >> beam.Map(lambda x: (int(x.decode('utf-8')) % 10, 'val'))
      ...
      lines_expected = {'counter: 110',
                        ...
                        'statecache:get_total: 100',
                        'statecache:miss_total: 10',
                        'statecache:hit_total: 90',
                        'statecache:put_total: 10',
                        'statecache:extend_total: 100',
                        'statecache:evict_total: 0',
                       }
      lines_actual = set()
